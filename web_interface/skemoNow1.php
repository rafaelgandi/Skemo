...
while($row1 = mysql_fetch_row($result0))
 {
  $result1 = mysql_query("SELECT subject_no from prospectus WHERE subject_no = '$row1[0]'");
   ...
   if(mysql_num_rows($result1) == 0) //not found
    {
	 $result2 = mysql_query("SELECT subject_title from common_data WHERE offer_no = $row1[1] and subject_id = $row1[2] limit 1;");   
...
{
	$sql = "Select subject_no from subjects_table where subject_no = '$value'";
	$result = mysql_query($sql);
...
foreach($forDisplay2 as $key => $value)
{
 ...
 if($ctr > 0)
 // -- updated 02-23-09 -- //
  echo "<td width='140' style = 'padding:14px 0 11px 0;'><input type=\"checkbox\" name='subjects[]' value='".$value."' id='".$value."' onchange='SkemoNow.util.unitsIndicator();' />$value</td>";
...
//{
	...
	/*
		...
		if($ctr > 0)
     	{
	 		echo "<td width='140' style = 'padding:14px 0 11px 0;'><input type=\"checkbox\" name='subjects[]' value='".$readySBJ[$ctrArray]."'/>";
			print_r($readySBJ[$ctrArray]);
			echo "</td>";
		}
...
<div style="vertical-align: bottom; text-align: left; padding-top: 25px; padding-left: 5px;">
	...
	<input type="checkbox" id="overload_indicator" name="overload_indicator" value="allow" />&nbsp; Allow unit overload.
	</span>
