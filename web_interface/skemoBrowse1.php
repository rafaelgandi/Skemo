...
if($_POST['searchSubmit'])
{
	...
	if(($courseNo != null) and ($subj_no == null))
	{
		if($courseNo == 'Guidance')
		{
			$result = mysql_query("SELECT offer_no, subject_no, time, day, room from subjects_table 
				where subject_no like '$courseNo%'");
		}
		
		else if($courseNo == 4)
			{
				$result = mysql_query("SELECT offer_no, subject_no, time, day, room from subjects_table 
				where offer_no like '$courseNo%' and subject_no not like 'Guid%'");
			}
		
		else
		{
			$result = mysql_query("SELECT offer_no, subject_no, time, day, room from subjects_table 
				where offer_no like '$courseNo%'");
	...
	{
		$result = mysql_query("SELECT offer_no, subject_no, time, day, room from subjects_table 
			where subject_no like '$subj_no%'");
	}

	else if(($courseNo != null) and ($subj_no != null))
	{
		$result = mysql_query("SELECT offer_no, subject_no, time, day, room from subjects_table 
			where subject_no like '$subj_no%' and offer_no like '$courseNo%'");
	}
